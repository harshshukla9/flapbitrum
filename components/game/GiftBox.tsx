...
export default function GiftBox({ onClose, onClaimComplete }: GiftBoxProps) {
  ...
  const getTokenAddress = (tokenType: 'arb' | 'pepe' | 'boop' | 'bribe' | 'none'): string => {
    switch (tokenType) {
      case 'arb':
  ...
  const getTokenInfo = (tokenType: 'arb' | 'pepe' | 'boop' | 'bribe' | 'none') => {
    switch (tokenType) {
      case 'arb':
      ...
      case 'bribe':
        return { name: '$bribe', color: 'text-pink-400', icon: '/candy/3.png' };
      case 'none':
          ...
          <div>
            ...
            <h2 style={{ 
              ...
              color: reward!.tokenType === 'arb' ? 'rgb(96, 165, 250)' :
                     ...
                     reward!.tokenType === 'bribe' ? 'rgb(244, 114, 182)' :
                     'rgb(156, 163, 175)'
            }}>
