...
export async function generateGiftBoxReward(score: number = 0): Promise<{
  ...
  if (tokenRandom < tokenChance) {
    ...
    const pepeAmount = 800 + Math.floor(Math.random() * (3778 - 1236 + 1));
    ...
    const boopAmount = 800 + Math.floor(Math.random() * (1000 - 411 + 1));
    ...
    bribe: 14000 - 23000
    const bribeAmount = 10000 + Math.floor(Math.random() * (23000 - 14000 + 1));
